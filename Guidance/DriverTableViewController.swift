...
class DriverTableViewController: UITableViewController, UISearchBarDelegate {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let carType = driver.carType != "" ? driver.carType : "Auto desconocido"
        cell.textLabel?.text = driver.name
        cell.detailTextLabel?.text = "\(carType), evaluado con \(driver.rate)"
    ...
    */
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        ...
        if searchText != "" {
            drivers = drivers!.filter({(d: Driver) -> Bool in
                return d.name.lowercased().range(of: (searchText.lowercased())) != nil
            })
        }
        tableView.reloadData()
    }
    ...
    @IBAction func cancelToDriversTableViewController(_ segue: UIStoryboardSegue) {
    }
    
    @IBAction func saveDriverDetail(_ segue: UIStoryboardSegue) {
        if let viewController = segue.source as? DriverDetailViewController {
